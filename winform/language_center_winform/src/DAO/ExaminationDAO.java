...
public class ExaminationDAO {
    ...
    public void insert(ExaminationDTO examination) throws Exception {
        HashMap<String, Object> newExamination = new HashMap<String, Object>();
        newExamination.put("name", examination.getName());
        newExamination.put("date", examination.getDate());
        HttpClient client = HttpClient.newHttpClient();
	    HttpRequest request = HttpRequest.newBuilder()
	            .uri(new URI(API_URL))
	            .headers("Content-Type", "application/json;charset=UTF-8")
	            .POST(HttpRequest.BodyPublishers.ofString(newExamination.toString()))
	            .build();
	    HttpResponse response = client.send(request, HttpResponse.BodyHandlers.ofString());
        JSONObject responseObj = new JSONObject(response.body().toString());
        System.out.println(responseObj);
    ...
    public void update(ExaminationDTO examination) throws Exception {
        HashMap<String, Object> updateExamination = new HashMap<String, Object>();
        updateExamination.put("name", examination.getName());
        updateExamination.put("date", examination.getDate());
        HttpClient client = HttpClient.newHttpClient();
	    HttpRequest request = HttpRequest.newBuilder()
	            .uri(new URI(API_URL))
	            .headers("Content-Type", "application/json;charset=UTF-8")
	            .PUT(HttpRequest.BodyPublishers.ofString(updateExamination.toString()))
	            .build();
	    HttpResponse response = client.send(request, HttpResponse.BodyHandlers.ofString());
        JSONObject responseObj = new JSONObject(response.body().toString());
        System.out.println(responseObj);
